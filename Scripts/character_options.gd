...
func _on_start_pressed() -> void:
	if player_1_selection == "" or player_2_selection == "":
		...
		var character_options = get_tree().root.get_node("Control")
		var myRoot = get_tree().root
		myRoot.remove_child(character_options)
		character_options.call_deferred("free")
		var switch = load("res://Scenes/stage.tscn").instantiate()
		myRoot.add_child(switch)
		switch.setPlayers(player_1_selection, player_2_selection)
